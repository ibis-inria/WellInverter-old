...
defaultOptions = {
	...
	plotOptions: {
		line: { // base series options
			...
			cropThreshold: 300, // drawMicroplate points outside the plot area when the number of points is less than this
			pointRange: 0,
			//pointStart: 0,
			//pointInterval: 1,
...
/**
 * A wrapper object for SVG selection
 */
function SVGElement() {}
...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * Set the coordinates needed to drawMicroplate a consistent radial gradient across
	...
	/**
	 * Apply CSS to HTML selection. This is used in text within SVG rendering and
	 * by the VML renderer
	 */
	...
	/**
	 * VML override private method to update selection based on internal
	 * properties based on SVG transform
	 */
	htmlUpdateTransform: function () {
		// aligning non added selection is expensive
		if (!this.added) {
	...
	updateTransform: function () {
		...
		// Apply translate. Nearly all transformed selection have translation, so instead
	...
	getBBox: function () {
		...
		if (!bBox) {
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				...
				// is for Opera that returns a width of -Infinity on hidden selection.
			...
			// True SVG selection as well as HTML selection in modern browsers using the .useHTML option
	...
	add: function (parent) {
		...
		// insert according to this and other selection' zIndex
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		// inside this container will be drawn at subpixel precision. In order to drawMicroplate
	...
	/**
	 * Detect whether the renderer is hidden. This happens when one of the parent selection
	...
	destroy: function () {
		...
		// Call destroy on all gradient selection
					...
					if (span !== ' ') {
						...
						if (width) {
							...
							while (words.length || rest.length) {
								...
								// Old IE cannot measure the actualWidth for SVG selection (#2314)
	...
	/**
	 * Resize the box and re-align all aligned selection
	 * @param {Object} width
	 * @param {Object} height
	...
	html: function (str, x, y) {
		...
		if (renderer.isSVG) {
			wrapper.add = function (svgGroupWrapper) {
				...
				// Create a mock group to hold the HTML selection
				if (svgGroupWrapper) {
...
Highcharts.VMLElement = VMLElement = {
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false && value !== null) { // #620
					...
					// text for rotated and non-rotated selection
					} else if (key === 'text') {
						this.bBox = null;
						element.innerHTML = value;
						skipAttr = true;
	...
	clip: function (clipRect) {
		...
		if (clipRect) {
			...
			erase(clipMembers, wrapper); // Ensure unique list of selection (#1258)
	...
	/**
	 * Apply a drop shadow by copying selection and giving them different strokes
	 * @param {Boolean|Object} shadowOptions
	 */
	shadow: function (shadowOptions, group, cutOff) {
		var shadows = [],
...
var VMLRendererExtension = { // inherit SVGRenderer
	 ...
	 * child HTML selection and CSS clip.
	 *
...
if (useCanVG) {
	...
	CanVGController = (function () {
		...
		/**
		 * When downloaded, we are ready to drawMicroplate deferred charts.
		 */
...
Axis.prototype = {
	...
	getOffset: function () {
		...
		// Create the axisGroup and gridGroup selection on first iteration
		if (!axis.axisGroup) {
	...
	render: function () {
		...
		// Mark all selection inActive before we go over and mark the active ones
		...
		// If the series has data drawMicroplate the ticks. Else only the line and title
		if (hasData || isLinked) {
...
Tooltip.prototype = {
	...
	/**
	 * Destroy the tooltip and its selection.
	 */
	destroy: function () {
...
Chart.prototype = {
	...
	redraw: function (animation) {
		...
		if (redrawLegend && legend.options.enabled) { // series or pie points are added or removed
			// drawMicroplate legend graphics
			legend.render();
...
Point.prototype = {
	...
	/**
	 * Destroy SVG selection associated with the point
	 */
...
Series.prototype = {
	...
	drawPoints: function () {
		...
		if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
			...
			while (i--) {
				...
				// only drawMicroplate the point if y is defined
	...
	destroy: function () {
		...
		// destroy all points with their selection
		i = data.length;
		while (i--) {
			point = data[i];
	...
	render: function () {
		...
		// SVGRenderer needs to know this before drawing selection (#1089, #1795)
		group.inverted = series.isCartesian ? chart.inverted : false;
		
		// drawMicroplate the graph if any
		if (series.drawGraph) {
			series.drawGraph();
			series.clipNeg();
		}

		// drawMicroplate the data labels (inn pies they go before the points)
		series.drawDataLabels();
		
		// drawMicroplate the points
		series.drawPoints();


		// drawMicroplate the mouse tracking area
		if (series.options.enableMouseTracking !== false) {
			series.drawTracker();
		}
...
var AreaSeries = extendClass(Series, {
	...
	/**
	 * For stacks, don't split segments on null values. Instead, drawMicroplate null values with
...
var ColumnSeries = extendClass(Series, {
	...
	translate: function () {
		...
		each(series.points, function (point) {
			var yBottom = pick(point.yBottom, translatedThreshold),
				plotY = mathMin(mathMax(-999 - yBottom, point.plotY), yAxis.len + 999 + yBottom), // Don't drawMicroplate too far outside plot area (#1303, #2241)
	...
	alignDataLabel: function (point, dataLabel, options,  alignTo, isNew) {
		var chart = this.chart,
			...
			inside = pick(options.inside, !!this.options.stacking); // drawMicroplate it inside the box?
...
var PiePoint = extendClass(Point, {
	...
	setVisible: function (vis) {
		...
		// Show and hide associated selection
...
var PieSeries = {
	...
	drawDataLabels: function () {
		...
		// Do not apply the final placement and drawMicroplate the connectors until we have verified
	...
	/**
	 * Verify whether the data labels are allowed to drawMicroplate, or we should run more translation and data
