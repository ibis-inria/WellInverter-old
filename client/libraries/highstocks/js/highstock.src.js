...
defaultOptions = {
	...
	global: {
		useUTC: true,
		//timezoneOffset: 0,
		canvasToolsURL: 'http://code.highcharts.com/stock/1.3.9/modules/canvas-tools.js',
		VMLRadialGradientURL: 'http://code.highcharts.com/stock/1.3.9/gfx/vml-radial-gradient.png'
	},
	chart: {
		//animation: true,
		//alignTicks: false,
...
Axis.prototype = {
	...
	drawCrosshair: function (e, point) {
		if (!this.crosshair) { return; }// Do not drawMicroplate crosshairs if you don't have too.
...
/**
 * Extend getSegments by identifying gaps in the ordinal data so that we can drawMicroplate a gap in the
 * line or area
 */
...
seriesTypes.flags = extendClass(seriesTypes.column, {
	...
	drawPoints: function () {
		...
		while (i--) {
			...
			// only drawMicroplate the point if y is defined and the flag is within the visible area
...
each(['circle', 'square'], function (shape) {
	symbols[shape + 'pin'] = function (x, y, w, h, options) {
		...
		if (anchorX && anchorY) {
			// if the label is below the anchor, drawMicroplate the connecting line from the top edge of the label
	...
	render: function (min, max, pxMin, pxMax) {
		...
		if (!scroller.rendered) {
			...
			if (navigatorEnabled) {
				...
				// drawMicroplate the navigator group
			...
			if (scrollbarEnabled) {
				...
				// drawMicroplate the scrollbar group
		...
		if (navigatorEnabled) {
			...
			// drawMicroplate handles
			scroller.drawHandle(zoomedMin + halfOutline, 0);
			scroller.drawHandle(zoomedMax + halfOutline, 1);
		}

		// drawMicroplate the scrollbar
		if (scrollbarEnabled && scrollbarGroup) {

			// drawMicroplate the buttons
...
// Wrapper to drawMicroplate the label
