...
var PlotController = (function () {
     ...
     * Constructor
     */
    function PlotController(wic) {
        /**
         * Y-axis scale type (scaleType[0]: left-most y-axis, scaleType[1]: right-most y-axis): "linear" | "logarithmic"
         */
        this.scaleType = ["linear", "linear"];
        /**
         * Colors used for plots
         */
        this.colors = ['#0099FF', '#00CC66', '#CC9933', '#CC3300', '#006600', '#000099', '#990066', '#CC6633', '#0066FF', '#009966', '#FF0000', '#999933', '#6600FF'];
        /**
         ...
         */
        this.currentColor = 'auto';
        /**
         * Point markers shapes used for plotting
         */
        this.markerSymbols = ["circle", "diamond", "triangle", "square", "triangle-down"];
        this.wic = wic;
    }
    /**
     * Update plots when an element of the plot selector has changed
     */
    PlotController.prototype.updatePlots = function () {
        var plotsCount = 0;
        var selectedWells = this.wic.plotSelector.selectedWells();
        this.updateColors(selectedWells);
        // Computation of the different y-axis
        var yAxis = []; // yAxis object
        var yAxisNames = []; // yAxis name
        var yAxisRefs = []; // reference to yAxis used for each series
        for (var p = 1; p <= this.wic.plotSelector.plotsCount; p++) {
            ...
            if (plotTypeText == "Protein concentration" || plotTypeText == "Promoter activity") {
                ...
                if (yAxisNames.indexOf(yAxisName) == -1) {
                    ...
                    yAxisRefs.push(yAxisNames.indexOf(yAxisName));
            }
            else {
                if (yAxisNames.indexOf(measureSubTypeText) == -1) {
                    ...
                    yAxisRefs.push(yAxisNames.indexOf(measureSubTypeText));
            }
            if (title != "") {
                if (yAxis.length == 0)
                    yAxis.push({ title: { text: title }, type: this.scaleType[0], labels: { formatter: function () {
                        return this.value;
                    } } });
                else
                    yAxis.push({ title: { text: title }, type: this.scaleType[1], opposite: true, labels: { formatter: function () {
                        return this.value;
                    } } });
            }
        }
     ...
     */
    PlotController.prototype.getCurve = function (measure, plotType) {
        switch (plotType) {
            case "originalSignal":
                return measure.getOriginalCurve();
            case "outlierFreeSignal":
                return measure.getOutlierFreeCurve();
            case "subtractedBackground":
                return measure.getSubtractedBackgroundCurve();
            case "synchronized":
                return measure.getSynchronizedCurve();
            case "growthRate":
                return measure.getGrowthRateCurve();
            case "reporterConcentration":
                return measure.getReporterConcentrationCurve();
            case "promoterActivity":
                return measure.getPromoterActivityCurve();
            case "":
                return new Curve([], []);
            default:
                throw new Error("Undefined plotType:" + plotType);
        }
    };
    /**
     * Return plot width
     */
    PlotController.prototype.getPlotLineWidth = function (plotType) {
        return (plotType == "originalSignal" ? 0 : 1);
    };
    /**
     * Refresh colorHash: remove colors for wells that are no longer selected
     */
    PlotController.prototype.updateColors = function (selectedWells) {
     ...
     */
    PlotController.prototype.getPlotColor = function (measure) {
     ...
     */
    PlotController.prototype.getMarkerSymbol = function (measure) {
     ...
     */
    PlotController.prototype.changeAxisScale = function (i) {
     ...
     */
    PlotController.prototype.showView = function () {
